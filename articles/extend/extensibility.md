...
which you can use them so that your own method is used to be
...
An example above doesn’t use one of built-in output S7 classes, so
[`tidy()`](https://joshuamarie.github.io/statim/reference/tidy.md) will
fail. Fortunately, you can bring it down by using
`making_tidy(<stat_fn>, <model_id / S7::class_formula>) %<-% method_tidy(...)`
method:
