...
[`via()`](https://s7-stats.github.io/statim/reference/via.md) is an S7
generic dispatched on the lazy object class and a character string. It
validates that the named variant exists for the current model type, then
writes the variant name and any additional arguments into
`@recalibrate_spec`:
...
Calling [`via()`](https://s7-stats.github.io/statim/reference/via.md) a
