...
Not all types of inference in R can be fully described by `<formula>`
...
`<model_id>` objects share this same lazy-capture nature — bare variable
...
while a `<model_id>` is always resolved the same way, through
...
`<model_id>` objects are built on top of S7, and serve as “mappers”,
similar to how `ggplot2::aes()` (aesthetic mappings) works. Where
`aes()` maps variables to plot aesthetics (`x`, `y`, `colour`, …), a
`<model_id>` maps variables to the roles a statistical model needs
(e.g. a response and a grouping variable for
...
Each of these is a `model_id` subclass, and each is paired with a
[`model_processor()`](https://s7-stats.github.io/statim/reference/model-processor.md)
...
Writing a new `<model_id>` object is easy but strict — it always
requires
[`model_processor()`](https://s7-stats.github.io/statim/reference/model-processor.md)
...
generic, it is not always required. The `model_id` is an abstract S7
class used as a parent class for `<model_id>` objects. The following
...
[`statim::model_id`](https://s7-stats.github.io/statim/reference/model_id.md),
and
[`statim::model_processor()`](https://s7-stats.github.io/statim/reference/model-processor.md).
For example, you want to describe another model that describes 3
variables at once during the analysis, naming this `<model_id>` object
`xyz`.

Two simple steps are required:

1.  To write a new `<model_id>` mapper, use
    [`S7::new_class()`](https://rconsortium.github.io/S7/reference/new_class.html)
    constructor, and put
    [`statim::model_id`](https://s7-stats.github.io/statim/reference/model_id.md)
...
2.  Extract the info you need by dispatching
    [`model_processor()`](https://s7-stats.github.io/statim/reference/model-processor.md)
    with the newly created `xyz` `<model_id>` object. Two arguments
...
If your `<model_id>` needs this “lazy” behavior — e.g. accepting bare
...
for the built-in `<model_id>` objects, handling bare names,
...
But for a first custom `<model_id>`, a simpler
...
class. Return a `<class_model_inform>` object via
...
4.  Optionally, register a
    ...
    `class_model_inform` object with `registered = TRUE`.

With these in place, your new `<model_id>` works seamlessly with
[`define_model()`](https://s7-stats.github.io/statim/reference/model-define-base.md)
...
[`rel()`](https://s7-stats.github.io/statim/reference/rel.md),
[`pairwise()`](https://s7-stats.github.io/statim/reference/pairwise.md),
and [`prop()`](https://s7-stats.github.io/statim/reference/prop.md).
